...
from ..tools.file.operations import (
    DeleteFileTool,
    ReadFileTool,
    SearchReplaceTool,
    WriteFileTool,
...
async def init_agent(config: dict, workspace_root: str = ".") -> AgentOrchestrator:
    ...
    # API ключ для embeddings (можно использовать тот же, что и для LLM)
    embedding_api_key = llm_config.get("api_key") or os.getenv("OPENAI_API_KEY")
    code_indexer = CodeIndexer(
        index_path=code_search_config.get("index_path", ".code_index"),
        embedding_model=code_search_config.get(
    ...
    if terminal_config.get("enabled", True):
        tool_registry.register(
            RunTerminalCmdTool(
                workspace, max_timeout=terminal_config.get("max_timeout", 300)
            )
        )
    ...
    if todo_config.get("enabled", True):
        tool_registry.register(
            TodoWriteTool(
                workspace, todo_file=todo_config.get("todo_file", ".todo.json")
            )
        )
...
async def cmd_execute(
    query: str, config_path: Optional[str] = None, verbose: bool = False
):
...
async def cmd_index(
    project_path: str = ".", config_path: Optional[str] = None, verbose: bool = False
):
    """Индексация проекта"""
    setup_logging(verbose)
    config = load_config(config_path)
