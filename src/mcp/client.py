...
class MCPClient:
    ...
    async def _initialize(self, name: str) -> None:
        ...
        if response.error:
            raise RuntimeError(f"Ошибка инициализации MCP сервера {name}: {response.error.message}")
    ...
    async def _load_tools(self, name: str) -> None:
        ...
        if response.error:
            logger.warning(f"Ошибка при загрузке инструментов с {name}: {response.error.message}")
            return
    ...
    async def _send_stdio_request(self, server: Dict[str, Any], request: MCPRequest) -> MCPResponse:
    ...
    def _convert_mcp_tool_to_openai(self, mcp_tool: Dict[str, Any], server_name: str) -> Dict[str, Any]:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        if "." not in tool_name:
            raise ValueError(f"Неверный формат имени инструмента: {tool_name}. Ожидается: server.tool")
        ...
        if response.error:
            raise RuntimeError(f"Ошибка при вызове инструмента: {response.error.message}")
